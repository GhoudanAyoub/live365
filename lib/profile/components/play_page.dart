...
class _PlayPageState extends State<PlayPage> {
  ...
  Widget _buildCard(int index) {
    ...
    String runtime;
    if (clip.mediaUrl.length > 60) {
      runtime = "${clip.mediaUrl.length ~/ 60}m ${clip.mediaUrl.length % 60}s";
    } else {
      runtime = "${clip.mediaUrl.length % 60}s";
    }
    return Card(
      elevation: 10,
      child: Container(
        ...
        child: Row(
          ...
          children: <Widget>[
            Padding(
              ...
              child: clip.mediaUrl.startsWith("https")
                  ? Image.network(clip.mediaUrl,
                      width: 70, height: 50, fit: BoxFit.fill)
                  : Image.asset(clip.mediaUrl,
                      width: 70, height: 50, fit: BoxFit.fill),
            ),
            Expanded(
              child: Column(

...
// Auth System
  ...
  static Future create(String email1, String pass, BuildContext context) async {
    try {
      userCredential = await FirebaseAuth.instance
          .createUserWithEmailAndPassword(email: email1, password: pass)
  ...
  static Future sign(String e, String pass, BuildContext context) async {
    try {
      userCredential = await FirebaseAuth.instance
          .signInWithEmailAndPassword(email: e, password: pass)
          .catchError((onError) => Scaffold.of(context).showSnackBar(
              SnackBar(content: Text("You Don't Have Account "))));
      return userCredential.user;
    } on FirebaseAuthException catch (e) {
  ...
  static Future FogetPassword(String email2, BuildContext context) async {
    try {
      await FirebaseAuth.instance
          ...
          .then((value) => {
                Scaffold.of(context).showSnackBar(SnackBar(
                    content: Text(
                        "We Send you A link To rest your password \n please Check Your Email")))
              });
